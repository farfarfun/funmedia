...
from funmedia.crawlers.bilibili.web.web_crawler import (
    BilibiliWebCrawler,
)  # 导入哔哩哔哩web爬虫
...
async def fetch_user_collection_videos(
    request: Request,
    folder_id: str = Query(
        example="1756059545", description="收藏夹id/collection folder id"
    ),
    pn: int = Query(default=1, description="页码/Page number"),
):
    """
...
async def fetch_collect_folders(
    request: Request, pn: int = Query(default=1, description="页码/Page number")
):
    """
    # [中文]
    ### 用途:
...
async def fetch_collect_folders(
    ...
    offset: str = Query(
        default="", example="953154282154098691", description="开始索引/offset"
    ),
):
    """
    # [中文]
