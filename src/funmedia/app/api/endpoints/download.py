...
from funmedia.app.api.models.APIResponseModel import ErrorResponseModel  # 导入响应模型
from funmedia.crawlers.hybrid.hybrid_crawler import HybridCrawler  # 导入混合数据爬虫
...
@router.get(
    "/download",
    summary="在线下载抖音|TikTok视频/图片/Online download Douyin|TikTok video/image",
)
async def download_file_hybrid(
    ...
    try:
        ...
        if data_type == "video":
            file_name = (
                f"{file_prefix}{platform}_{aweme_id}.mp4"
                if not with_watermark
                else f"{file_prefix}{platform}_{aweme_id}_watermark.mp4"
            )
            url = (
            ...
            __headers = (
                ...
                if platform == "tiktok"
                else await HybridCrawler.DouyinWebCrawler.get_douyin_headers()
            )
        ...
        # 下载图片文件/Download image file
            ...
            zip_file_name = (
                f"{file_prefix}{platform}_{aweme_id}_images.zip"
                if not with_watermark
                else f"{file_prefix}{platform}_{aweme_id}_images_watermark.zip"
            )
            ...
            for url in urls:
                ...
                file_name = (
                    f"{file_prefix}{platform}_{aweme_id}_{index + 1}.{file_format}"
                    if not with_watermark
                    else f"{file_prefix}{platform}_{aweme_id}_{index + 1}_watermark.{file_format}"
                )
                file_path = os.path.join(download_path, file_name)
