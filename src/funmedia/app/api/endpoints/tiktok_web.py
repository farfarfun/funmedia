...
from funmedia.app.api.models.APIResponseModel import (
    ResponseModel,
    ErrorResponseModel,
)  # 导入响应模型
...
async def fetch_user_post(
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_user_post(
...
async def fetch_user_like(
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_user_like(
            secUid, cursor, count, coverFormat
        )
        return ResponseModel(code=200, router=request.url.path, data=data)
    except Exception as e:
...
async def fetch_user_collect(
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_user_collect(
...
async def fetch_post_comment(
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_post_comment(
            aweme_id, cursor, count, current_region
        )
        return ResponseModel(code=200, router=request.url.path, data=data)
    except Exception as e:
        status_code = 400
...
async def fetch_post_comment_reply(
    ...
    comment_id: str = Query(
        example="7304877760886588191", description="评论id/Comment id"
    ),
    cursor: int = Query(default=0, description="翻页游标/Page cursor"),
    count: int = Query(default=20, description="每页数量/Number per page"),
    current_region: str = Query(default="", description="当前地区/Current region"),
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_post_comment_reply(
            item_id, comment_id, cursor, count, current_region
        )
        return ResponseModel(code=200, router=request.url.path, data=data)
    except Exception as e:
        status_code = 400
...
async def fetch_user_fans(
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_user_fans(
            secUid, count, maxCursor, minCursor
        )
        return ResponseModel(code=200, router=request.url.path, data=data)
    except Exception as e:
        status_code = 400
...
async def fetch_user_follow(
    ...
    """
    try:
        data = await TikTokWebCrawler.fetch_user_follow(
            secUid, count, maxCursor, minCursor
        )
        return ResponseModel(code=200, router=request.url.path, data=data)
    except Exception as e:
